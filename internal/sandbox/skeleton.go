...
# Initialize oh-my-posh prompt theme engine if theme is present
...
func appendDynamicConfig(cfg *config.Config, bashrcPath string) error {
	...
	if enableOMP {
		themePath := "~/.mytheme.omp.json"
		if cfg.OhMyPosh != nil && cfg.OhMyPosh.ThemePath != nil && *cfg.OhMyPosh.ThemePath != "" {
			themePath = *cfg.OhMyPosh.ThemePath
		}
		themePathCheck := themePath
		if strings.HasPrefix(themePathCheck, "~/") {
			themePathCheck = "$HOME/" + themePathCheck[2:]
		}
		data, err := os.ReadFile(bashrcPath)
		if err == nil && !strings.Contains(string(data), "oh-my-posh init") {
			sb.WriteString("\n# Initialize oh-my-posh prompt theme engine (Generated by bws from configuration file 'features.enable_oh_my_posh' or 'oh_my_posh' entry)\n")
