...
`, shellType, themePath, home, home, shellType, themePath)
	...
	comment := "# Initialize oh-my-posh prompt theme engine (Generated by bws from configuration file 'features.enable_oh_my_posh' or 'oh_my_posh' entry)"
...
func injectCdtoday(targetFile, base string) {
	...
	re := regexp.MustCompile(`(?m)(?:# Function to navigate to today's.*\n)?cdtoday \(\) \{\n(?:.*\n)*?\}\n?`)
	content = re.ReplaceAllString(content, "")
...
`, base)
	...
	comment := fmt.Sprintf("# Function to navigate to today's %s directory (Generated by bws from configuration file 'cdtoday' entry)", base)
